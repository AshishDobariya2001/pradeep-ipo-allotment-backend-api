...
export class MaashitlaService {
  ...
  async getAllotmentStatus(ipo?: IpoDetailsDto) {
    try {
      ...
      console.log(
        '🚀 ~ MaashitlaService ~ getAllotmentStatus ~ companyList:',
        companyList,
      );
      ...
      console.log(
        '🚀 ~ MaashitlaService ~ GetAllotmentStatus ~ foundIpo:',
        foundIpo,
      );

      // if (foundIpo && !ipo.ipoAllotmentRequiredPayload) {
      //   const result = await this.ipoDetailsRepository.update(ipo.id, {
